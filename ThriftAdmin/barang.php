...
<head>
  ...
  <title>Table Barang Dashboard</title>
...
<section class="content">
  <div class="container-fluid">
    ...
    <div class="row mb-2">
      <div class="col-sm-6">
        <a href="add_barang.php" class="btn btn-primary">Tambah Data Barang</a>
      </div><!-- /.col -->
      <div class="col-sm-6">
...
$total_rows_query = "SELECT COUNT(*) as total_rows FROM barang";
...
$sql = "SELECT barangid, jenis_barang, nama, stock, harga FROM barang LIMIT $offset, $per_page";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<td>";
        echo "<a href='edit_barang.php?id={$row['barangid']}' class='btn btn-primary btn-sm ml-2'>Edit</a>";
        echo "<a href='delete_barang.php?id={$row['barangid']}' class='btn btn-danger btn-sm ml-2'>Hapus</a>";
        echo "</td>";
        echo "</tr>";
    }
} else {
...
<script>
  ...
  document.getElementById('searchByIdButton').addEventListener('click', function() {
    ...
    searchBarang('barangid', searchId);
  });
  ...
  function searchBarang(filter, keyword) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState === 4 && xhr.status === 200) {
        document.getElementById('barangTable').getElementsByTagName('tbody')[0].innerHTML = xhr.responseText;
      }
    };
    xhr.open('POST', 'search_barang.php', true);
